...
class GUI_Grupos(ttk.Frame):
    def __init__(self,notebook,main,client):
        ...
        self.cajaCupo = ttk.Spinbox(self,width=13,from_=5, to=45,state="disabled")
        ...
        self.btnBaja["command"] = self.BajaGrupo
    ...
    def GuardarGrupo(self):
        ...
        if Carrera and NombreGrupo and Semestre and Fecha and Cupo:
            ...
            if self.GrupoTemporal == None:    
                ...
                if sql.CheckGrupo(self.Grupo.get_Grupo(),None):
                ...
                self.GrupoTemporal.set_Fecha(self.cajaFecha.get_date())
                self.GrupoTemporal.set_Grupo(self.cajaGrupo.get().strip())
                if sql.CheckGrupo(self.GrupoTemporal.get_Grupo(),self.GrupoTemporal.get_IDGrupo()):
                    messagebox.showerror("Error al Guardar","Ya existe un Grupo con el mismo nombre, asegurese de cambiarlo por uno nuevo.")
                    return
                sql.ActualizarGrupoInfo(self.GrupoTemporal)
                self.CancelarCampos(None,True)
                messagebox.showinfo("Éxito al Guardar","El Grupo ha sido editado con éxito.") 
    ...
    def BajaGrupo(self):
        if messagebox.askyesno("Baja de Grupo","Estas seguro de que quieres dar de baja este grupo?\nAl hacerlo, todos los horarios y alumnos de este volveran a tener disponibilidad para su nueva asignación.\nIMPORTANTE: Esta acción no puede ser revertida"):
            sql.BajaGrupo(self.GrupoTemporal)
            messagebox.showinfo("Baja de Grupo","El Grupo seleccionado ha sido dado de baja.")
            self.CancelarCampos(None,True)
