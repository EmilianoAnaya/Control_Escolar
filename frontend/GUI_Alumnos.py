...
from frontend.GUI_Prerregistro import GUI_Prerregistro
...
class GUI_Alumnos(ttk.Frame):
    def __init__(self,notebook,main,client):
        ...
        self.client = client
        self.bind("<Visibility>", lambda event: self.ActualizarFrame(event,main,client))
        ...
        self.btnPrerregistro = ttk.Button(self,text="Prerregistro", padding=(7,5), state="disabled")
        self.btnPrerregistro.place(x=305,y=320)
        ...
        self.btnPrerregistro["command"] = lambda: Tools.AbrirVentana(GUI_Prerregistro(self,self.AlumnoTemporal),self.btnPrerregistro)
    ...
    def ActualizarFrame(self,event,main,client):
        Tools.AjustarResolucion(event,"700x390",main)
        self.CarrerasDisponibles = sql.get_Carreras()
        self.cajaCarrera["values"] = self.CarrerasDisponibles
        if client.get_Perfil() == "alumno":
            self.cajaBuscarAlumno.insert(0,client.get_Username())
            self.BuscarAlumnos()
            self.cajaBuscarAlumno.delete(0,END)
        if client.get_Perfil() == "alumno" or client.get_Perfil() == "maestro":
    ...
    def CancelarCampos(self,event,bandera):
        ...
        self.btnEditar["state"]="disabled"
        self.btnBaja["state"]="disabled"
        self.btnPrerregistro["state"]="disabled"
    ...
    def BuscarAlumnos(self):
        if self.cajaBuscarAlumno.get().strip() != "":
            ...
            if self.AlumnoTemporal != None:
                ...
                self.btnPrerregistro["state"]="normal"
            else:
        ...
        self.cajaBuscarAlumno.delete(0,END)
        if self.client.get_Perfil() == "maestro":
            self.btnBaja["state"]="disabled"
            self.btnEditar["state"]="disabled"
            self.btnNuevo["state"]="disabled"
            self.btnCancelar["state"]="disabled"
