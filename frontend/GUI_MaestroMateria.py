...
class GUI_MaestroMaterias(tk.Toplevel):
    ...
    def BajaAsignaciones(self,id_maestro):
        if messagebox.askyesno("Baja Asiganciones","Estas seguro de querer eliminar las materias y carreras asignadas a este profesor?"):
            if self.MateriasActuales != None and self.CarrerasActuales != []:
                if not sql.BajasMaestro([carrera_id for carrera_id in self.CarrerasActuales],id_maestro,"maestro_carreras","id_carrera"):
                    messagebox.showerror("Error al Editar","Algunas carreras que se quisieron dar de baja cuentan con un horario al nombre del maestro, siendo esta edición no éxitosa")
                    self.CancelarCampos()
                    return
                if not sql.BajasMaestro([materia_id for materia_id in self.MateriasActuales],id_maestro,"maestro_materias","id_materia"):
                    messagebox.showerror("Error al Editar","Algunas materias que se quisieron dar de baja cuentan con un horario al nombre del maestro, siendo esta edición no éxitosa")
                    self.CancelarCampos()
                    return
    ...
    def GuardarAsignaciones(self, id_maestro):
        ...
        if CarrerasBaja:
            if not sql.BajasMaestro(CarrerasBaja,id_maestro,"maestro_carreras","id_carrera"):
                messagebox.showerror("Error al Editar","Algunas carreras que se quisieron dar de baja cuentan con un horario al nombre del maestro, siendo esta edición no éxitosa")
                return
        if MateriasBaja:
            if not sql.BajasMaestro(MateriasBaja,id_maestro,"maestro_materias","id_materia"):
                messagebox.showerror("Error al Editar","Algunas materias que se quisieron dar de baja cuentan con un horario al nombre del maestro, siendo esta edición no éxitosa")
                return
        if CarrerasAlta:
