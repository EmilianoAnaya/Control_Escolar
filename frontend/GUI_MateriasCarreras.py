...
class GUI_MateriasCarreras(tk.Toplevel):
    ...
    def BajaCarreras(self,id_materia):
        if messagebox.askyesno("Baja Carreras","Estas seguros de que quieres dar de baja la disponibilidad de esta materia en todas las carreras?"):
            if self.CarrerasActuales != None:
                if not sql.BajaMateriasCarreras([carrera_id for carrera_id in self.CarrerasActuales], id_materia):
                    messagebox.showinfo("Error al Baja","La Materia no pudo eliminarse debido a que esta tiene horarios ya disponibles.")
                    self.CancelarCampos()
                    return
    ...
    def GuardarCarreras(self,id_materia):
        ...
        if CarrerasBaja:
            if not sql.BajaMateriasCarreras(CarrerasBaja,id_materia):
                messagebox.showerror("Error al Editar","Algunas carreras que se quisieron dar de baja ya tienen un horario con la materia asignada, siendo esta edicion cancelada.")
                self.CancelarCampos()
                return
        if CarrerasAlta:
