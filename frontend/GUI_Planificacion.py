...
class GUI_Planificacion(ttk.Frame):
    def __init__(self,notebook,main,client):
        ...
        self.bind("<Visibility>",lambda event: self.ActualizarPantalla(event,main,client))
    ...
    def ActualizarPantalla(self,event,main,client):
        Tools.AjustarResolucion(event,"720x560",main)
        if client.get_Perfil() == "alumno" or client.get_Perfil() == "maestro":
            self.cajaBuscarHorarios.insert(0,client.get_Username())
            self.BuscarHorarios()
            self.cajaBuscarHorarios.delete(0,END)