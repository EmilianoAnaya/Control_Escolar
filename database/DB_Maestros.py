...
from database.DB_Herramientas import ValidarUsername, BuscarUsuario, ValidarNombreCompleto,CheckEliminacion
...
def MaestroBaja(IDMaestro):
    if not CheckEliminacion("id_maestro",IDMaestro):
        conn, cursor = abrir_bd()
        cursor.execute(f"DELETE FROM maestro_materias WHERE id_maestro = '{IDMaestro}' ")
        cursor.execute(f"DELETE FROM maestro_carreras WHERE id_maestro = '{IDMaestro}' ")
