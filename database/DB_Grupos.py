...
def GuardarDatos(ListaAlumnos, Grupo):
    ...
    if len(HorariosSeleccionados) >= 4:
        ...
        if len(AlumnosGrupo) >= int(0.6 * Grupo.get_Cupo()):
            cursor.execute(f'''INSERT INTO grupos(id_grupo, id_carrera, grupo, semestre, fecha, cupo, cupos_disponibles) 
                           VALUES ('{Grupo.get_IDGrupo()}','{Grupo.get_IDCarrera()}','{Grupo.get_Grupo()}','{Grupo.get_Semestre()}','{Grupo.get_Fecha()}'
...
def BajaGrupo(Grupo):
    conn, cursor = abrir_bd()
    cursor.execute(f"SELECT id_horario FROM horarios_grupo WHERE id_grupo = '{Grupo.get_IDGrupo()}' ")
    IDHorarios = [res[0] for res in cursor.fetchall()]
    cursor.execute(f"SELECT id_alumno FROM alumnos_grupo WHERE id_grupo = '{Grupo.get_IDGrupo()}' ")
    IDAlumnos = [res[0] for res in cursor.fetchall()]
    cursor.execute(f"UPDATE alumnos SET id_grupo = 0 WHERE id_grupo = '{Grupo.get_IDGrupo()}' ")
    for id in IDHorarios:
        cursor.execute(f"UPDATE horarios SET disponibilidad = 1 WHERE id_horario = '{id}' ")
        cursor.execute(f"DELETE FROM horarios_grupo WHERE id_horario = '{id}' ")

    for id in IDAlumnos:
        cursor.execute(f"DELETE FROM alumnos_grupo WHERE id_alumno = '{id}' ")

    cursor.execute(f"DELETE FROM grupos WHERE id_grupo = '{Grupo.get_IDGrupo()}' ")
    conn.commit()
    cerrar_bd(conn)


def ActualizarGrupoInfo(Grupo):
    conn, cursor = abrir_bd()
    cursor.execute(f"UPDATE grupos SET grupo = '{Grupo.get_Grupo()}', fecha = '{Grupo.get_Fecha()}' WHERE id_grupo = '{Grupo.get_IDGrupo()}'")
    conn.commit()
    cerrar_bd(conn)
...
def CheckGrupo(grupo,id_grupo):
    conn, cursor = abrir_bd()
    if id_grupo != None:
        CadenaExtra = f"AND NOT id_grupo = {id_grupo}"
    else:
        CadenaExtra = ";"
    cursor.execute(f"SELECT * FROM grupos WHERE grupo = '{grupo}'{CadenaExtra} ")
    res = cursor.fetchone()
    cerrar_bd(conn)
    if res == None:
        return False
