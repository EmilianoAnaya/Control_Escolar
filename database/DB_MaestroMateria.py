...
def CheckAsignaciones(Asignaciones,id_maestro,id_busqueda):
    conn, cursor = abrir_bd()
    for id_asignacion in Asignaciones:
        cursor.execute(f"SELECT * FROM horarios WHERE {id_busqueda} = '{id_asignacion}' AND id_maestro = '{id_maestro}' ")
        res = cursor.fetchone()
        if res == None:
            continue
        else:
...
def BajasMaestro(Asignaciones,id_maestro,tabla:str,id_busqueda:str):
    if CheckAsignaciones(Asignaciones,id_maestro,id_busqueda):
        conn, cursor = abrir_bd()
