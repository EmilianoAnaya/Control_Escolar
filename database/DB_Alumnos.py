...
def BajaAlumno(IDAlumno):
    conn, cursor = abrir_bd()
    cursor.execute(f"UPDATE alumnos SET status = 0, id_grupo = 0, prerregistro = 0 WHERE id_alumno = '{IDAlumno}'")
    cursor.execute(f"DELETE FROM alumnos_grupo WHERE id_alumno = '{IDAlumno}' ")
    cursor.execute(f"DELETE FROM prerregistro WHERE id_alumno = '{IDAlumno}' ")
    conn.commit()
    cerrar_bd(conn)