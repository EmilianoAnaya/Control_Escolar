...
def CheckCarreras(Carreras,id_materia):
    conn, cursor = abrir_bd()
    for id_carrera in Carreras:
        cursor.execute(f"SELECT * FROM horarios WHERE id_carrera = '{id_carrera}' AND id_materia = '{id_materia}' ")
        res = cursor.fetchone()
        if res == None:
            continue
        else:
...
def BajaMateriasCarreras(Carreras,id_materia):
    if CheckCarreras(Carreras,id_materia):
        conn,cursor = abrir_bd()
        for id_carrera in Carreras:
