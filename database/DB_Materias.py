...
from database.DB_Herramientas import CheckEliminacion
...
def BajaMateria(IDMateria):
    if not CheckEliminacion("id_materia",IDMateria):
        conn,cursor = abrir_bd()
        cursor.execute(f"DELETE FROM materias_carreras WHERE id_materia = '{IDMateria}'")
